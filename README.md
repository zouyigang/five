...
- **奖励检验**
  - GUI 中的奖励检验页可以：
    ...
    - 明细包含：形成活四/冲四/活三/跳活三/眠三、封堵对方已有威胁、未化解对方威胁、错失直接获胜落点等。
...
- **奖励检验**：
  - 在棋盘上落子即可查看该步总奖励与明细（形成活三/眠三、封堵对方活四、未压制对方活三、错失直接获胜落点等）。
...
- **对局记录**：`runs/<run_id>/games/<game_id>.json`
  ...
  - `moves[]` 数组中每一项为一个 `MoveRecord`：
    ...
    - `total_reward`、`reward_details`：该步奖励总分与明细（形成活三、封堵对方活四、错失直接获胜落点等）
...
按本手落子后**新形成**的棋形给分，统一乘以系数 `attack_scale`（默认 0.02）。棋形基础分（未乘系数前）如下：
...
实际奖励 = 基础分 × 数量 × `attack_scale`，再与过程奖励上下限裁剪。

### 二、防守奖励（化解对方**当前盘面已有**的威胁）

只统计对方**已经摆在盘上**的威胁（活四、冲四、活三、跳活三等），不统计“对方下一手才能形成的威胁”。本手若落在可化解这些威胁的格点上，按化解的威胁类型与权重 × `block_scale`（默认 0.018）加分。

### 三、漏防惩罚（对方已有威胁未化解）

若对方当前盘面存在活四、冲四、活三等威胁，本手**没有**落在可化解这些威胁的位置，则按未化解的威胁类型扣分（如未化解对方活四、未压制对方活三等）。惩罚力度大于同级的防守奖励，避免“只顾进攻不防守”。
