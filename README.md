...
- GUI 中的人机对弈页可以：
  ...
  - 选择人类执黑/执白。
  - 选择**搜索强度**（即难度）：**关闭(直出)** / **快(64)** / **标准(200)** / **强(800)**。开启后用 MCTS 搜索落子，棋力显著高于网络直出。单局对弈用不上跨对局批量，每次模拟即一次前向，本机实测约 3 / 113 / 390 / 1983 ms 一手。状态栏会显示本手的搜索强度、局面估值与首选手的访问占比。
  - 选择**开局随机**手数：**关闭** / **1 手** / **2 手** / **4 手**。AI 的前 N 手按分布采样，之后全程取最优手。推理本该全程最优，但那样同一开局会走出完全相同的一盘棋（实测 24 局只有 1 局不同），人摸到一条赢棋路线就能无限重放；把随机性限制在开局，中后盘棋力一点不损失。
