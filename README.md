...
- `test_threat_shapes.py`：棋形检测自测（成五、活四、冲四、活三、眠三、跳活三、跳活四、跳冲四及 open_ends 不重复计数等）。

## 奖励系统

训练使用 PPO 塑形奖励 + 终局奖励，引导模型既重视当前棋形又重视胜负。单步奖励由 **进攻奖励**、**防守奖励**、**漏防惩罚**、**错失己方赢棋点惩罚** 组成，并做裁剪；终局仅在获胜步给获胜奖励，并在最后若干步叠加衰减的胜负回传（outcome tail bonus）。
