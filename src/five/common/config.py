...
class RewardConfig:
    ...
    # ---------- 每步时间成本 ----------
    # 过程塑形项净值恒为正，不抵消的话「继续拖」本身就在赚钱：策略要么拖满棋盘求和
    # （ppo_kl run：对局 56->73 手、胜率归零），要么在赢棋奖励放大后发现「输棋也净赚」
    # 而彻底放弃防守（ppo_econ run：执白 24 战全负、挡住率 59%->33%）。
    #
    # 取值按 ppo_econ 的 464 局实测：步均过程奖励（不含本项）= +0.2496。取 0.20 略低于
    # 实测值，留一点正净值，避免塑形波动时净值转负、反过来激励「尽快输」。
    #
    # 它是每步常数偏移，不改变同一局面下各落点的相对优劣（argmax 不变），只改变
    # 「结束对局」与「继续拖」的相对价值——正是需要调的那个维度。
    time_step_penalty: float = 0.20
    # ---------- 开局 ---------- 前 N 步内生效的位置奖惩
    ...
    # 终局：获胜/平局/失败时的额外奖励，加在获胜手 / 输家最后一手上。
    #
    # 必须**对称**。这一项曾被关成 0.0（当时开 3.0 会导致 48 局 42 和，因为过程分
    # 净值为正、拖长有利可图，求和优于求胜）——但那是 time_step_penalty 不足造成的，
    # 关掉它只是换来另一个极端：赢 +8.0 而输 0.0，按实测步均 +0.20 算，输一局净收益
    # 仍有 +1.8，防守彻底失去经济价值，实测执白 24 战全负、挡住率 59%->33%。
    # 正确做法是把「拖」的收益用 time_step_penalty 抵掉，再让输赢对称定价。
    #
    # 配 time_step_penalty=0.20 后的账（单方约 9 手）：
    #   速胜 ~ +8.5   拖满和棋 ~ +2.0   速败 ~ -7.6
    final_win_reward: float = 8.0
    final_loss_penalty: float = 8.0
...
class TrainingConfig:
    ...
    # 对锚点策略（run 起点的冻结副本）的 KL 惩罚系数，0 = 关闭。
    # 从一个好的监督起点做 RL 时，绝大多数梯度来自「普通走子」这类噪声样本
    # （实测占 93.7%，归一化优势均值 -0.025、标准差 0.945），而真正携带战术信息的
    # 样本只占 0.2%，压不住噪声，策略会被推散（单局面熵 0.18 -> 3.74，防守准确率
    # 81% -> 1.7%）。KL 项让策略只在有明确收益时才偏离起点。
    # 40 轮四组对照（判据为强制局面下的防守准确率 + 胜/和/负分解）：
    #   无 KL      挡住率 19.2%  对 BC 16胜2和30负  对启发式 24胜0和24负
    #   kl=0.1     挡住率 80.4%  对 BC 21胜27和0负  对启发式 24胜24和0负
    #   kl=0.5     挡住率 78.0%  对 BC  6胜42和0负  对启发式  0胜48和0负（约束过强，学不动）
    kl_coef: float = 0.1
